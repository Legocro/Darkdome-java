...
public abstract class Player {
   ...
   public boolean inFishingTrawlerRewardsInterface;
	...
	protected void appendPlayerAppearance(Stream str) {
		...
		if (playerEquipment[playerHead] > 1) {
			playerProps.writeWord(0x200 + playerEquipment[playerHead]);
		} else {
			playerProps.writeByte(0);
		}

		if (playerEquipment[playerBack] > 1) {
			playerProps.writeWord(0x200 + playerEquipment[playerBack]);
		} else {
			playerProps.writeByte(0);
		}

		if (playerEquipment[playerNeck] > 1) {
			playerProps.writeWord(0x200 + playerEquipment[playerNeck]);
		} else {
			playerProps.writeByte(0);
		}

		if (playerEquipment[playerRightHand] > 1) {
			playerProps.writeWord(0x200 + playerEquipment[playerRightHand]);
		} else {
			playerProps.writeByte(0);
		}
		...
		if (playerEquipment[playerLeftHand] > 1) {
			playerProps.writeWord(0x200 + playerEquipment[playerLeftHand]);
		} else {
			playerProps.writeByte(0);
		}
		...
		if (!Item.isFullHelm(playerEquipment[playerHead])
				&& !Item.isFullMask(playerEquipment[playerHead])) {
	...
	public boolean wearing2h() {
		Client c = (Client) this;
		String s = ItemAssistant.getItemName(c.playerEquipment[c.playerRightHand]);
		if (s.contains("2h")) {
			return true;
